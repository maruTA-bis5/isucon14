...
(
  ...
  COMMENT = '椅子情報テーブル';
CREATE INDEX chair_access_token ON chairs (access_token);
CREATE INDEX chair_owner ON chairs (owner_id);
...
CREATE INDEX chair_location_chair_id_created_at ON chair_locations (chair_id, created_at);
...
(
  ...
  COMMENT = 'ライド情報テーブル';
CREATE INDEX ride_chair_id ON rides (chair_id);
CREATE INDEX ride_user_id ON rides (user_id);
...
CREATE INDEX ride_status_ride_id_created_at ON ride_statuses (ride_id, created_at);
...
(
  ...
  COMMENT 'クーポンテーブル';
CREATE INDEX coupon_used_by ON coupons (used_by);
CREATE INDEX coupon_user_id ON coupons (user_id);
